# NOTE: Code was adjusted from https://google.github.io/mesop/demo/ (Fancy Chat)
# To run the app: `mesop optimaizer/app.py`
import time
...
from optimaizer.main import start_pricing_agent
...
_APP_TITLE = "Pricing Optimizer Agent"
_BOT_AVATAR_LETTER = "🤑"
_EMPTY_CHAT_MESSAGE = "Get started with an example"
_EXAMPLE_USER_QUERIES = (
    "Optimize pricing for product-A using current inventories and market sizes.",
    "What is the current inventory levels for product-B?",
    "What is a good pricing strategy for product-C if inventory increases by 20%?",
)
...
def respond_to_chat(input: str, history: list[ChatMessage]):
    ...
    # ⬇ This emulates a stream of responses from the agent
    #   (we did not implement streaming in the agent in order to keep function calling simple)
    chunk_size = 5  # Number of characters per "fake stream" step
    for i in range(0, len(response), chunk_size):
        yield response[i : i + chunk_size]
        time.sleep(random.uniform(0.02, 0.04))  # Simulate network latency
...
def bot_message(*, message_index: int, message: ChatMessage):
    with me.box(
        ...
        classes=["bot-message"],  # Custom CSS class used for interacting with Selenium
    ):
        text_avatar(
    ...
    with me.box(
        ...
        with me.box(
            ...
            me.native_textarea(
                ...
                placeholder="Chat with the pricing agent",
                style=me.Style(
                    background=me.theme_var("surface-container")
                    if _is_mobile()
                    else me.theme_var("surface-container"),
...
def on_click_history(e: me.ClickEvent):
    ...
    chat_messages = [ChatMessage(**chat) for chat in state.history.pop(int(chat_index))]
    if state.output:
        state.history.insert(0, [asdict(messages) for messages in state.output])
...
def on_click_regenerate(e: me.ClickEvent):
    ...
    output_message = respond_to_chat(user_message.content, state.output[:msg_index])
